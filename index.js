...
var EncyptionManager = require("bcryptjs");
var NodeExpressServer = express();
var ServerConfigs = require("http").createServer(NodeExpressServer);
var MainEventListner = require("socket.io").listen(ServerConfigs);
var bodyParser = require("body-parser");
var DatabseServer = require("mongodb").DatabseServer;
var DatabaseConfig =
  "mongodb+srv://ksbm:Ravi1234@cluster0.rneyjw2.mongodb.net/test1";
var ServerRes = require("./ServerResponse");

NodeExpressServer.use(bodyParser.json());
NodeExpressServer.use(
  bodyParser.urlencoded({
    extended: true,
  })
);

NodeExpressServer.set("port", process.env.PORT || 8001);

NodeExpressServer.get("/", function (req, res) {
...
var currVersion = 5;
var apkUrl =
  "https://drive.google.com/file/d/164_IrSfwkNe6xqX318MHDMJ6u9qVZZOj/view?usp=sharing";
...
NodeExpressServer.get("/online", function (req, res) {
  res.json(clients);
});
    ...
    if (ch3) {
      ...
      if (lSocket.adapter.rooms[lSocket.room] != undefined) {
        if (lSocket.adapter.rooms[lSocket.room].play == "1") {
          ch3 = false;
          console.log(
            "sds " + lSocket.adapter.rooms[lSocket.room].searchPlayers
          );
          ...
          for (var j in PList) {
            ...
            if (
              ...
              lSocket2.adapter.rooms[lSocket2.room].play == "1"
            ) {
              if (lSocket2.bot == 0) {
                var sta;
                ...
                lSocket2.emit("Player_Connect", {
                  ...
                  watch: lSocket2.watch,
                  standby: lSocket2.standby,
                  status: sta,
                });
                lSocket2.broadcast
                  .in(lSocket2.room)
                  .emit("OtherPlayer_Connect", {
                    name: lSocket2.username,
                    total_chips: lSocket2.total_chips,
                    socket_id: lSocket2.id,
                    sittingPos: lSocket2.seat - 1,
              ...
              if (lsearch == 0) {
                if (Total_Player(lSocket2.room, 3) <= sitPos) {
                  ...
                  else lSocket.adapter.rooms[lSocket.room].play = "0";
                }
              }
            }
          }
        ...
        if (socRoom.play == "2" && socRoom.searchOne == 0) {
          ...
          if (socRoom.gameTimer > 5) {
            ...
            if (socRoom.length >= 2) {
              for (var k in PList) {
                ...
                if (sroom.play == "2" && lSocket2.room == lSocket3.room) {
                  ...
                  lSocket3.emit("StartBootAmount", {
                    ...
                    seat: lSocket3.seat - 1,
                    total_chips: lSocket3.total_chips,
                  });
              ...
              for (var k in PList) {
                var lSocket3 = PList[k];
                if (socRoom.dealerValue == lSocket3.seat - 1) ccVal1 = true;
                if (socRoom.curPlyValue == lSocket3.seat - 1) ccVal2 = true;
              }
              if (!ccVal1) nextDealerValue(socRoom, lSocket2);
              if (!ccVal2) nextCurrPlayer(socRoom, lSocket2);
            } else {
              ch2 = false;
              socRoom.play = "0";
            }
          ...
          for (var k in PList) {
            ...
            if (sroom.play == "3" && lSocket2.room == lSocket3.room) {
              ...
              lSocket3.broadcast
                .in(lSocket3.room)
                .emit("PassStr", { seat: lSocket3.seat - 1 });
            }
          }
    ...
    if (socRoom != undefined && lSocket2.watch == 0) {
      ...
      if (socRoom.play == "5" && cChe) {
        ...
        if (socRoom.gameTimer == 0) {
          chipsCheck = tccfxn(socRoom, lSocket2);
          console.log(
            "karthik " +
              chipsCheck +
              " " +
        ...
        if (intvalue >= 20) {
          ...
          if (lSocket2.out == 1) {
            ...
            lSocket2.broadcast
              .in(lSocket2.room)
              .emit("EndGameTimer", { cPlay: socRoom.curPlyValue });
            if (intvalue > 20) {
              lSocket2.emit("RedGameTimerClose", {
                cPlay: socRoom.curPlyValue,
              });
              lSocket2.broadcast
                .in(lSocket2.room)
                .emit("RedGameTimerClose", { cPlay: socRoom.curPlyValue });
              var pp = checkPacked(lSocket2);
              if (pp == 1) lSocket2.adapter.rooms[lSocket2.room].play = "6";
            }
            if (chipsCheck == 0) {
              lSocket2.pack = 1;
              ...
              lSocket2.broadcast
                .in(lSocket2.room)
                .emit("CallPack", { seat: lSocket2.seat - 1 });
            ...
            if (intvalue == 20) {
              ...
              lSocket2.broadcast
                .in(lSocket2.room)
                .emit("RedGameTimer", { cPlay: socRoom.curPlyValue });
            } else if (intvalue >= 30) {
              lSocket2.emit("RedGameTimerClose", {
                cPlay: socRoom.curPlyValue,
              });
              ...
              lSocket2.broadcast
                .in(lSocket2.room)
                .emit("CallPack", { seat: lSocket2.seat - 1 });
              /*var roomCount2 = PList[j].adapter.rooms[PList[j].room];
							if (roomCount2 != undefined)
								console.log("dddd " + roomCount2.length);*/
              //delete PList[j];
              lSocket2.emit("TimeOut", {
                nochips: "You are Timed Out",
                time: "1",
              });
            }
          }
        ...
        if (socRoom.waitingCount == 0) {
          ...
          if (tCount >= 2) {
            ...
            lSocket2.broadcast
              .in(lSocket2.room)
              .emit("Instruction", { instr: " Next Round is Start" });
            ...
            for (var k in PList) {
              ...
              if (lSocket3.room == lSocket2.room) {
                ...
                if (lSocket3.standby == 1) lSocket3.watch = 1;
                lSocket3.emit("OtherPlayer_Connect", {
                  name: lSocket3.username,
                  total_chips: lSocket3.total_chips,
                  socket_id: lSocket3.id,
                  sittingPos: lSocket3.seat - 1,
                  watch: lSocket3.watch,
                  status: "yes",
                  standby: lSocket3.standby,
                });
                lSocket3.broadcast
                  .in(lSocket3.room)
                  .emit("OtherPlayer_Connect", {
            ...
            lSocket2.emit("Instruction", {
              instr: " No Other Player to Start",
            });
            lSocket2.broadcast
              .in(lSocket2.room)
              .emit("Instruction", { instr: "  No Other Player to Start" });
          }
        }
      }
    }
...
function sffxn(s) {
  ...
  if (seenCount == 2 && totalPla == 2) seenCount = 2;
  else seenCount = 0;
  return seenCount;
}
  ...
  while (eChe) {
    ...
    for (var i in PList) {
      ...
      if (
        ...
        lSocket.room == lSocket2.room &&
        localCPlay != socRoom.dealerValue
      ) {
        eChe = false;
      }
    }
  ...
  while (eChe) {
    ...
    for (var k in PList) {
      ...
      if (
        ...
        localCPlay != socRoom.curPlyValue &&
        lSocket4.watch == 0 &&
        lSocket4.pack == 0
      )
        eChe = false;
  ...
  for (var i in PList) {
    ...
    if (socRoom != undefined) {
      if (lSocket2.room == lSocket.room && winPlay == lSocket.seat - 1) {
        ...
        lSocket.broadcast
          .in(lSocket.room)
          .emit("Instruction", { instr: "Maximum pot reached" });
      }
    }
  }
}
function Total_Player(droom, side) {
  ...
  for (var i in PList) {
    ...
    } else if (side == 2) playerCount += 1;
    else if (side == 3) {
      if (lSocket.room == droom) {
        if (lSocket.standby == 0) playerCount += 1;
      }
    }
  }
  return playerCount;
...
MainEventListner.on("connection", function (socket) {
  ...
  socket.on("Room", function (data) {
    ...
    if (alreadyPlay) {
      ...
      if (roomSocket == undefined) {
        ...
        if (
          roomSocket.length < 6 &&
          data.room == socket.adapter.rooms[i + ""].roomid
        ) {
          socket.join(i + "");
          socket.emit("RoomConnected", { room: parseInt(i + "") });
          ch2 = false;
  ...
  socket.on("Player_Connect", function (data) {
    ...
    if (socket.standby == undefined) socket.standby = 0;
    socket.carStr1 = "";
    socket.carStr2 = "";
    socket.carStr3 = "";
    ...
    socket.bot = 0;
    if (
      socket.adapter.rooms[data.room].play == "2" ||
      socket.adapter.rooms[data.room].play == "3" ||
      socket.adapter.rooms[data.room].play == "4" ||
      socket.adapter.rooms[data.room].play == "5"
    )
      socket.watch = 1;
    else socket.watch = 0;
    ...
    for (var i in PList) {
      if (ch2) {
        ...
        if (
          ...
          lSocket.broadcast
            .in(socket.room)
            .emit("CallShow", { seat: lSocket.seat - 1 });
          ch2 = false;
        }
      }
    }
  ...
  socket.on("ACCEPT", function (data) {
    ...
    for (var k in PList) {
      ...
      if (socket.room == lSocketShow.room && lSocketShow.watch == 0) {
        if (
          lSocketShow.pack == 0 &&
          (lSocketShow.seat - 1 == data.ask || lSocketShow.seat - 1 == data.you)
        ) {
          var handNormal = scoreHandsNormal([
            lSocketShow.carStr1,
            lSocketShow.carStr2,
    ...
    for (var i in PList) {
      if (ch2) {
        ...
        if (
          ...
          lSocket.broadcast
            .in(socket.room)
            .emit("CallPack", { seat: lSocket.seat - 1 });
          ch2 = false;
        }
      }
    }
        ...
        if (socRoom.play == "0") {
          ...
          if (socket.id == lSocket.id && socket.watch == 0) {
            ...
            if (lSocket.sideshow == 1) {
              ...
              socket.broadcast
                .in(socket.room)
                .emit("SideShowRemove", { asked: lSocket.asked });
            }
            if (totPCount > 2) {
              socRoom.gameTimer = 0;
              ...
              socket.broadcast
                .in(lSocket.room)
                .emit("Instruction", { instr: lSocket.name + " is out" });
            }
          }
    ...
    for (var i in PList) {
      ...
      if (socRoom != undefined && lSocket.room == socket.room) {
        if (socRoom.play == "1") {
          ...
          socket.broadcast
            .in(socket.room)
            .emit("CallPack", { seat: socket.seat - 1 });

          socket.broadcast
            .in(lSocket.room)
            .emit("Instruction", { instr: lSocket.name + " is out" });
          var pp = checkPacked(lSocket);
          if (pp == 1) lSocket.adapter.rooms[socket.room].play = "6";
        } else if (socRoom.play == "5") {
          if (socket.id == lSocket.id && socket.watch == 0) {
            lSocket.pack = 1;
            var pp = checkPacked(lSocket);
...
function GetTotalUsers(lSocket) {
  pool.query("SELECT * FROM categories", function (err, result, fields) {
    lSocket.emit("TotalUser", {
      users: result.length,
      currUsers: Total_Player(0, 2),
    });
  });
}
...
function WithdrawSearch(lSocket, data) {
  ...
  pool.query(sql, [data.email], function (error, result, fields) {
    for (var i in result) {
      lSocket.emit("WithdrawSearch", {
        withdraw_limit: result[i].withdraw_limit,
      });
    }
  });
}
    ...
    dbo
      ...
      .updateOne(myquery, newvalues, function (error, result) {
        if (error) {
          ...
          //console.log("update success");
          lSocket.emit("ChangeMobile", {
            status: "success",
            changeStr: data.editColName,
            passStr: data.editPassStr,
          });
        }
        db.close();
      });
...
function PrivateTable(lSocket, data) {
  ...
  var post = {
    room_id: rValue,
    email: data.email,
    table_name: data.tableType,
    boot_value: parseInt(data.BootAmt),
    max_blind: parseInt(data.MaxBlind),
    table_player: 8,
    created_At: today,
  };
  pool.query(
    "INSERT INTO private_table SET ?",
    post,
    function (error, result, fields) {
      if (error) {
      } else {
...
function JoinRoom(lSocket, data) {
  ...
  pool.query(sql, [data.room_id], function (error, result, fields) {
    if (error) {
      ...
      console.log("avai 2");
      for (var i in result)
        lSocket.emit("JoinPrivateRoom", {
          id: result[i].room_id,
          BootValue: result[i].boot_value,
          MaxBlind: result[i].max_blind,
...
function GetCommission(lSocket) {
  pool.query("SELECT * FROM settings", function (err, result, fields) {
    lSocket.emit("Settings", {
      commission: result[0].conversion_rate,
      bonusRate: result[0].currency,
      private_price: result[0].question_time,
...
function scoreHandsNormal(playerCards) {
  if (playerCards.length == 3) {
    ...
    if (sameNumberCount == 2) {
      ...
      handStatus.no = 1;
      for (var i = 0; i < 3; i++) {
        if (playerCards[i].charAt(1) == "s") handStatus.no += 0.2;
        else if (playerCards[i].charAt(1) == "h") handStatus.no += 0.15;
        else if (playerCards[i].charAt(1) == "d") handStatus.no += 0.1;
        else if (playerCards[i].charAt(1) == "c") handStatus.no += 0.05;
      }
    ...
    if (sameColorCount == 1 && isSequence) {
      if (playerCards[0].charAt(1) == "s") handStatus.no = 4.2;
      else if (playerCards[0].charAt(1) == "h") handStatus.no = 4.15;
      else if (playerCards[0].charAt(1) == "d") handStatus.no = 4.1;
      else if (playerCards[0].charAt(1) == "c") handStatus.no = 4.05;
